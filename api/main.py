...
async def search_files(
    ...
    mtime_from: Optional[int] = Query(None, description="Minimum modification time (Unix timestamp)"),
    mtime_to: Optional[int] = Query(None, description="Maximum modification time (Unix timestamp)"),
    ...
    per_page: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE, description="Results per page")
):
    ...
    search_query, count_query = build_search_query(
        q, mode, ext, dir, mtime_from, mtime_to,
        size_min, size_max, sort, page, per_page
    )
...
async def get_stats():
    ...
    roots_result = execute_sql(
        "SELECT root, COUNT(*) as count, SUM(size) as size "
        "FROM files GROUP BY root"
    )
    roots = []
