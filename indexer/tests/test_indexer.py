...
class TestFileIndexer:
    ...
    def test_bulk_upsert(self, mock_post, indexer):
        ...
        rows = [
            (1, "test", "/path/file1.txt", "file1.txt", "txt", "/path", 100, 1000000, 1000, 1000, 755, 123456),
            (2, "test", "/path/file2.py", "file2.py", "py", "/path", 200, 2000000, 1000, 1000, 755, 123456),
        ]
    ...
    def test_sweep_deletions(self, mock_post, indexer):
        ...
        assert mock_post.called
        call_args = mock_post.call_args
        assert f"DELETE FROM files WHERE root='test' AND seen_at < {scan_id}" in call_args[1]['json']['query']
